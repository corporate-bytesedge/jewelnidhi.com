...
# {{ config('custom.mail_message_title_order_packaged') }}

![]({{asset('/img/'.config('custom.mail_logo'))}})

{{ config('custom.mail_message_order_packaged') }} @lang('Your Order Number is :order_id', ['order_id'=>$order->getOrderId()]).
@if( !empty ( $order->tracking_id ) )
    @lang('You can Track your Order using tracking id :tracking_id', ['tracking_id'=>$order->tracking_id]).
@endif
<hr>
