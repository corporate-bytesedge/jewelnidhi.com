...
@section('content')
    ...
    <div class="container">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ strtolower($product->category->category->name) }}">{{ $product->category->category->name }}</a></li>
        @endif
        @if(isset($product->category->name) && $product->category->name!='')
        <li class="breadcrumb-item"><a href="{{ strtolower($product->category->name) }}">{{ $product->category->name }}</a></li>
        @endif
        ...
        <div class="col-md-7">
          <div class="product-details-desc">
           ...
           @if($product->old_price)
                <span class="price-old"><del>&#8377; {{ number_format($product->old_price,2) }}</del></span>
            @endif  
                <span class="price-regular">&#8377; {{ isset($product->new_price) ? number_format($product->new_price,2) : '' }}</span>
           </div>
            ...
            <div class="pro-size">
              ...
              <select class="nice-select groupVal">
                ...
                @if($groupArr)
                  ...
                  @foreach($groupArr AS $k=> $value)
                       
                      @foreach($value AS $ke=> $valu)
                        <option  data-product_id = "{{ $valu['product_id'] }}" value="{{ $valu['size'] }}"> 
                        {{ $valu['size'] }}
                         </option>
                      @endforeach
       ...
       <div class="col-md-6">
         <div class="product-desc">
           ...
           <table class="table table-bordered">
             ...
             <tr>
              <td>Height</td>
              <td id="product_height">{{ number_format($product->product_height,2) }} mm</td>
            </tr>
            @endif
            @if(isset($product->product_width))
            <tr>
              <td>Width</td>
              <td id="product_width">{{ number_format($product->product_width,2) }} mm</td>
            </tr>
            @endif
            @if(isset($product->total_weight))
            <tr>
              <td>Product Weight (Approx.)</td>
              <td id="total_weight">{{ $product->total_weight }}  gram</td>
            </tr>
            @endif
           </table>
           ...
           <tr>
                ...
                <td>{{ $product->silverItem['rate'] ? number_format($product->silverItem['rate'],2) : '0.00'  }}</td>
              </tr>
           @endif
           ...
           @if(isset($product->gold_price))
             <tr>
               <td>Gold</td>
               <td> Rs. {{ number_format($product->gold_price,2)  }} </td>
             </tr>
          @endif
          @if(isset($product->vat_rate))
             <tr>
              <td>VAT</td>
              <td id="vat_rate"> Rs. {{ number_format($product->vat_rate,2) }} </td>
            </tr>
          @endif
            <!-- <tr>
          ...
          @if(isset($product->total_weight))
            <tr>
              <td>Total Weight</td>
              <td id="approxtotal_weight">  {{ number_format($product->total_weight,2) }} gram </td>
            </tr>
          @endif
          @if(isset($product->gst_three_percent))
            <tr>
              <td>GST</td>
              <td id="gst_three_percent"> Rs. {{ number_format($product->gst_three_percent,2) }} </td>
            </tr>
          @endif
          @if(isset($product->carat_weight))
            <tr>
              <td>Carat Weight</td>
              <td id="carat_weight"> Rs. {{ number_format($product->carat_weight,2) }} </td>
            </tr>
          @endif
          @if(isset($product->per_carate_cost))
            <tr>
              <td>Per Carate Cost </td>
              <td> Rs. {{ number_format($product->per_carate_cost,2) }} </td>
            </tr>
          @endif
          @if(isset($product->total_price))
            <tr>
              <td>Total</td>
              <td id="total_price"> Rs. {{ number_format($product->total_price,2) }} </td>
            </tr>
          @endif
           </table>
                  ...
                  <div class="product-carousel-4 slick-row-10 slick-arrow-style">
                    <div class="cat-item">
                      ...
                      <figure class="product-thumb">
                        <a href="#">
                            <img class="pri-img" src="{{ URL::asset('img/product-4.jpg') }} " alt="product">
                            <img class="sec-img" src="{{ URL::asset('img/product-15.jpg') }}" alt="product">
                        </a>
                        ...
                        <div class="cart-hover">
                            <button class="btn btn-cart">View Details</button>
                        </div>
                    </figure>
              ...
              if($(this).val()!='') {
                  $.ajax({
                    ...
                    success:function(result) {
                      ...
                      var keys = [];
                      var values = [];
                      $("#new_price").val(result.total_price);
