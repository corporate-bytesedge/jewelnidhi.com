...
<?php $__env->startSection('title'); ?>
    <?php echo app('translator')->getFromJson('Add Product'); ?>
...
<?php $__env->startSection('page-header-description'); ?>
    <?php echo app('translator')->getFromJson('Add New Product'); ?> <a href="<?php echo e(url()->previous()); ?>"><?php echo app('translator')->getFromJson('Go Back'); ?></a>
<?php $__env->stopSection(); ?>
...
<?php $__env->startSection('scripts'); ?>
    ...
    <script>
        ...
        var loadingMessage = '<span class="text-primary"><?php echo app('translator')->getFromJson('Loading...'); ?></span>';
        var errorMessage = '<span class="text-danger"><?php echo app('translator')->getFromJson('Loading...'); ?></span>';
        $('.existing_product_box').dropdown({
            ...
            choice: function (e) {
                ...
                if(productID) {
                    ...
                    $.ajax({
                        type: "get",
                        url: "<?php echo e(url('manage/existing-product')); ?>/" + productID,
                        success: function(response) {               
                            if(response) {
        ...
        $(document).ready(function() {
            ...
            <?php if(old('category')): ?>
                $("#category").val("<?php echo e(old('category')); ?>");
            <?php endif; ?>
        });
                ...
                if(!receivedData.error) {
                    if(receivedData.data.length != 0) {
                        ...
                        receivedData.data.forEach(function(element) {
                            specificationsHTML += '<tr> <td> <select class="form-control selectpicker" data-style="btn-default" name="specification_type[]"><option value="' +element.id+ '">'+element.name+'</option>'+options+'</select> </td> <td> <input class="form-control" placeholder="<?php echo app('translator')->getFromJson('Example: 14, 3.5, red'); ?>" name="specification_type_value[]" type="text"> </td> <td> <input class="form-control" placeholder="<?php echo app('translator')->getFromJson('kg, GHz (Leave blank if no unit)'); ?>" name="specification_type_unit[]" type="text"> </td> <td> <button class="remove_row btn btn-danger btn-xs" type="button"> <span class="glyphicon glyphicon-remove" aria-hidden="true"></span> </button> </td> </tr>'
                        });
                        $('.specification_types_rows').html(specificationsHTML);
                    } else {
                        $('<p class="text-danger"><?php echo app('translator')->getFromJson('Specifications are not set for this category.'); ?></p>').insertAfter(getSpecifications).fadeOut(2500);
                    }
                } else {
                    $('<p class="text-danger"><?php echo app('translator')->getFromJson('Error occured while fetching specifications for this category.'); ?></p>').insertAfter(getSpecifications).fadeOut(2500);
                }
            });
        });
...
<?php $__env->startSection('content'); ?>
    <?php echo $__env->make('partials.manage.products.create', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
<?php $__env->stopSection(); ?>
