...
class FrontController extends Controller
{
    ...
    public function search(Request $request) {
        ...
        $subproductid =  \DB::table('products')->select(\DB::raw("GROUP_CONCAT(id) AS product_ids"))->whereRaw('is_approved = 1 AND is_active = 1 AND ('. implode(' or ',$productSubQueryArr).')')->first();  
        ...
        if(is_null($catSql->rootcatids) && is_null($subCatSql->subcatids) && is_null($subproductid->product_ids)) {
           ...
           //dd($subproductid->product_ids);
            if(!is_null($subproductid->product_ids)  ) {
                 
                $products->orWhereRaw('product_category_styles.product_id  IN('.$subproductid->product_ids.')');
            }
            
            
            if(!is_null($subproductid->product_ids) && (!is_null($catSql->rootcatids) || !is_null($subCatSql->subcatids))) {
